...
public class TheVariables {
	public static class WorldVariables extends WorldSavedData {
		public static final String DATA_NAME = "the_worldvars";
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				The.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(1, this));
			} else {
				The.PACKET_HANDLER.send(PacketDistributor.DIMENSION.with(world.dimension::getType), new WorldSavedDataSyncMessage(1, this));
			}
		}
	...
	public static class MapVariables extends WorldSavedData {
		public static final String DATA_NAME = "the_mapvars";
		public MapVariables() {
			super(DATA_NAME);
		}
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				The.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(0, this));
			} else {
				The.PACKET_HANDLER.send(PacketDistributor.ALL.noArg(), new WorldSavedDataSyncMessage(0, this));
			}
		}
	...
	public static class WorldSavedDataSyncMessage {
		...
		private static void syncData(WorldSavedDataSyncMessage message, LogicalSide side, World world) {
			if (side.isServer()) {
				...
				if (message.type == 0) {
					The.PACKET_HANDLER.send(PacketDistributor.ALL.noArg(), message);
					world.getServer().getWorld(DimensionType.OVERWORLD).getSavedData().set(message.data);
				} else {
					The.PACKET_HANDLER.send(PacketDistributor.DIMENSION.with(world.dimension::getType), message);
